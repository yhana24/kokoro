...
module.exports["config"] = {
  ...
  credits: "Cliff",
  info: `art canvas`,
  usages: "Mention or reply a photo.",
  cd: 5,
  aliases: [],
};
...
module.exports["run"] = async function({ api, event, args }) {
  try {
    ...
    let responseUrl;
    if (mentionID) {
      responseUrl = `${devs.cliffcan}/${name}?userid=${mentionID}`;
    } else if (imageUrl) {
      responseUrl = `${devs.cliffcan}/${name}?image=${encodeURIComponent(imageUrl)}`;
    } else if (url) {
