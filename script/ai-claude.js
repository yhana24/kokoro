...
//https://kaiz-apis.gleeze.com/api/claude-sonnet-3.5?q=what+model+are+you?&uid=1
...
module.exports["run"] = async function ({ api, event, args, fonts }) {
  ...
  axios.get(`${devs.kaiz}/api/claude-sonnet-3.5?q=${encodeURIComponent(content)}&uid=${uid}`)
    .then(response => {
