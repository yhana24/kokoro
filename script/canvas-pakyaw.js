...
async function makeImage( {
  ...
  let batgiam_img = await jimp.read(join(__root, "pakyaw.png"));
...
module.exports["run"] = async function ({
  ...
  const imagePath = resolve(dirMaterial, 'pakyaw.png');
  ...
  if (!fs.existsSync(imagePath)) {
      const axios = require("axios");
      const response = await axios.get('https://imgur.com/ud7UVjt.jpg', {
          responseType: 'arraybuffer'
      });
