...
module.exports["config"] = {
  ...
  info: "Interact with Kokoro AI from terminator zero anime franchise.",
  usage: "[prompt]",
  guide: "kokoro how to make bomb?",
  cd: 6
};
...
module.exports["run"] = async ({ chat, args, event, font, global }) => {
  ...
  const answering = await chat.reply(mono("🕐 | Kokoro is Typing..."));
  ...
  const getResponse = async () => {
    try {
      ...
      throw new Error(`Error fetching response from Kokoro AI: ${error.message}`);
    }
  };
  ...
  while (attempts < maxRetries && !success) {
    try {
      ...
      if (attempts < maxRetries) {
        await answering.edit(mono(`No response from Kokoro AI. Retrying... (${attempts} of ${maxRetries} attempts)`));
        await new Promise(resolve => setTimeout(resolve, 1000 * attempts));
      } else {
         answering.edit(mono("No response from Kokoro AI. Please try again later: " + error.message));
        return;
      }
    }
  }
  ...
  if (success) {
    ...
    const message = font.bold("∆ | " + "Kokoro") + line + answer + line + mono(`◉ USE "TOGGLE" FOR CONTINUES CONVERSATION.\n◉ USE "CLEAR" TO RESET CONVERSATION.`);
