...
module.exports["run"] = async ({ api, event, args }) => {
  ...
  if (args.length < 2) {
   ...
   const { data } = await axios.get(`https://tenor.googleapis.com/v2/featured?key=&contentfilter=high&media_filter=png_transparent&component=proactive&collection=emoji_kitchen_v5&q=${encodeURIComponent(emoji1)}_${encodeURIComponent(emoji2)}`);
