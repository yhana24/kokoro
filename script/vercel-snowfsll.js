...
module.exports.run = async function({ api, event, args }) {
  try {
    ...
    const imageUrlFromResponse = response.data.imageUrl;

    if (!imageUrlFromResponse) {
      return api.sendMessage('Failed to retrieve the image URL from the API response.', event.threadID, event.messageID);
    }

    const imageResponse = await axios.get(imageUrlFromResponse, { responseType: 'stream' });
