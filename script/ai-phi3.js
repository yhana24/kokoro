...
module.exports["config"] = {
  ...
  info: "Interact with PHI 3 AI chatbot.",
  usage: "[prompt]",
  guide: "phi3 hello how are you?",
  cd: 6
};
...
module.exports["run"] = async ({ chat, args, event, font, global }) => {
  ...
  const mistral_model = models.microsoft[1];
  ...
  const answering = await chat.reply(mono("🕐 | PHI 3 is Typing..."));
  ...
  const getResponse = async () => {
    try {
      ...
      throw new Error(`Error fetching response from PHI AI: ${error.message}`);
    }
  };
  ...
  while (attempts < maxRetries && !success) {
    try {
      ...
      if (attempts < maxRetries) {
        await answering.edit(mono(`No response from PHI AI. Retrying... (${attempts} of ${maxRetries} attempts)`));
        await new Promise(resolve => setTimeout(resolve, 1000 * attempts));
      } else {
         answering.edit(mono(`No response from PHI AI. Please try again later: ${error.message}`));
        return;
      }
    }
  }
