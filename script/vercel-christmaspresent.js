...
module.exports.run = async function({ api, event, args }) {
  try {
    ...
    let responseUrl;
    if (mentionID) {
      responseUrl = `https://api-canvass.vercel.app/christmas-present?userid=${mentionID}`;
    } else if (imageUrl) {
      responseUrl = `https://api-canvass.vercel.app/christmas-present?image=${encodeURIComponent(imageUrl)}`;
    } else if (url) {
      ...
      responseUrl = `https://api-canvass.vercel.app/christmas-present?image=${imgurLink}`;
    }
