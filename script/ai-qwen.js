...
module.exports["config"] = {
  ...
  info: "Interact with Qwen AI. Multi-variations model.",
  usage: "[prompt]",
  guide: "qwen How does nuclear fusion work?",
  cd: 6
};
...
module.exports["run"] = async ({ chat, args, event, font, global }) => {
  ...
  var qwen_model = models.qwen[2];
  var name = qwen_model.split('/').pop().toUpperCase();
  ...
  const getResponse = async () => {
    return axios.post(url + qwen_model, {
  ...
  while (attempts < maxRetries && !success) {
    try {
      ...
      if (attempts < maxRetries) {
        await answering.edit(mono(`No response from Qwen AI. Retrying... (${attempts} of ${maxRetries} attempts)`));
        await new Promise(resolve => setTimeout(resolve, 1000 * attempts));
      } else {
         answering.edit(mono("No response from Qwen AI. Please try again later: " + error.message));
        return;
      }
    }
  }
