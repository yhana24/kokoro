...
module.exports["config"] = {
  name: "sqlcoder",
  isPrefix: false,
  aliases: ["sqlc", "sqlai", "sqlgen", "aisql"],
  version: "1.0.0",
  credits: "Kenneth Panio",
  role: 0,
  type: "artificial-intelligence",
  info: "Interact with Sqlcoder AI. Multi-variations model.",
  usage: "[prompt]",
  guide: "sqlcoder How does nuclear fusion work?",
  cd: 6
};
...
module.exports["run"] = async ({ chat, args, event, font, global }) => {
  ...
  var cf_random = Math.floor(Math.random() * models.sqlcoder.length);
  var random_models = models.sqlcoder[cf_random];
  var name = random_models.split('/').pop().toUpperCase();
  ...
  const getResponse = async () => {
    return axios.post(url + random_models, {
  ...
  while (attempts < maxRetries && !success) {
    try {
      ...
      if (attempts < maxRetries) {
        await answering.edit(mono(`No response from Sqlcoder AI. Retrying... (${attempts} of ${maxRetries} attempts)`));
        await new Promise(resolve => setTimeout(resolve, 1000 * attempts));
      } else {
         answering.edit(mono("No response from Sqlcoder AI. Please try again later: " + error.message));
        return;
      }
    }
  }
