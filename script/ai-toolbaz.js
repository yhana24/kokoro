...
module.exports["config"] = {
        name: "toolbaz",
        aliases: ["tb", "tbpro", "toolbazpro"],
        isPrefix: false,
        version: "1.0.0",
        credits: "Kenneth Panio",
        role: 0,
        type: "artificial-intelligence",
        info: "Interact with ai ToolbazPro.",
        usage: "[prompt]",
        guide: "toolbaz hello how are you?",
        cd: 6
};
...
module.exports["run"] = async ({ chat, args, event, font, global }) => {
  ...
  const answering = await chat.reply(mono("🕐 | Toolbaz is Typing..."));
  ...
  const getResponse = async () => {
      ...
      throw new Error(`Error fetching response from Toolbaz AI: ${error.message}`);
    }
  };
  ...
  while (attempts < maxRetries && !success) {
    try {
      ...
      if (attempts < maxRetries) {
        await answering.edit(mono(`No response from Toolbaz AI. Retrying... (${attempts} of ${maxRetries} attempts)`));
        await new Promise(resolve => setTimeout(resolve, 1000 * attempts));
      } else {
         answering.edit(mono(`No response from Toolbaz AI. Please try again later: ${error.message}`));
        return;
      }
    }
  }
