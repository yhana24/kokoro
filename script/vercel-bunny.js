...
module.exports["run"] = async function({ api, event, args }) {
  try {
    ...
    let responseUrl;
    if (mentionID) {
      responseUrl = `${devs.cliffcan}/bunny?userid=${mentionID}`;
    } else if (imageUrl) {
      responseUrl = `${devs.cliffcan}/christmas-present?image=${encodeURIComponent(imageUrl)}`;
    } else if (url) {
      ...
      responseUrl = `${devs.cliffcan}/bunny?image=${imgurLink}`;
    }
